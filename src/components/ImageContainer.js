...
function ImageContainer({ imageUrl }) {
  ...
  // Prevents to re-generate after every interaction with rating stars
  useEffect(() => {
      ...
      <figure>
        ...
        <div className="image-container__info">
          ...
          <div className="image-container__rating" onClick={handleRatingClick}>
            <div className={`image-container__popup ${showPopup ? 'rating-show-popup' : ''}`}>
              <p>Rated!</p>
            </div>
            <StarRating
              count={5}
        ...
        <div className="image-container__favorite">
          ...
          <AiFillHeart className={`image-container__heart-icon ${favoriteClicked ? 'favorite-clicked' : ''}`} onClick={handleFavoriteClick}/>
        </div>
      </figure>
    </div>
  );
